...
- **`Settings.qml`** (`settings` entry point) — lists detected + previously-configured devices and lets the user set per device: `enabled`, `icon`, `iconColor`, `ringColor`, `launchCmd`. Settings shape: `pluginSettings.devices` is a map keyed by device `id`; globals are `refreshInterval` and `showPercentage`. Edits are staged in a working copy and only committed in `saveSettings()`.
