...
Item {
  ...
  function getResults(query) {
    ...
    if (!query || query.trim() === "") {
      // Return filtered apps, prioritized by usage, then category weights
      const categoryWeights = {
        "WebBrowser": 2000,
        "Network": 1900,
        "Chat": 1800,
        "InstantMessaging": 1800,
        "Development": 1700,
        "IDE": 1700,
        "TextEditor": 1600,
        "Virtualization": 1500,
        ...
        "Utility": 500,
        "System": 400,
        "Settings": -5000, // Force to bottom
        "DesktopSettings": -5000,
        "HardwareSettings": -5000
      };

      function getWeight(app) {
        const cats = getAppCategories(app);
        const name = (app.name || "").toLowerCase();
        
        // Hard-relegate settings
        if (cats.includes("Settings") || cats.includes("DesktopSettings") || cats.includes("HardwareSettings") || 
            name.includes("setting") || name.includes("config") || name.includes("setup")) {
          return -5000;
        }

        // Special handling for requested apps
        if (name.includes("vmware") || name.includes("virtual")) return 1500;
        if (name.includes("todo") || name.includes("tradehub")) return 1400;
        if (name.includes("neovim") || name.includes("code") || name.includes("emacs") || name.includes("obsidian")) return 1700;

        let maxWeight = 500; // Default for Utility/Misc
        for (let cat of cats) {
          if (categoryWeights[cat] !== undefined) {
            maxWeight = Math.max(maxWeight, categoryWeights[cat]);
          }
        }
        return maxWeight;
      }
      ...
      const sorted = filteredEntries.slice().sort((a, b) => {
                                              ...
                                              // 1. Force settings to bottom (weight difference is huge)
                                              if (wa <= -5000 && wb > -5000) return 1;
                                              if (wb <= -5000 && wa > -5000) return -1;

                                              // 2. High weight categories (Browsers, Chat, Dev)
                                              if (wa !== wb) {
                                                // If one is significantly higher weight, prioritize it
                                                if (Math.abs(wa - wb) >= 100) return wb - wa;
                                              }

                                              // 3. Most used within similar priority
                                              ...
                                              // 5. Tie-break with specific weights
